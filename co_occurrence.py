...
USAGE
    ...
    # Twitter Example
    python -m jobs.twitter.co_occurrence \
        --input "data/election2020/#2020election_20190616.json" \
        --output "output" \
        --lang "en" \
        --interval "daily" \
        --topics_file "resources/20200715_blm_topics.json" \
        --preprocessing_choices 'lowercase' 'stopwords'
    ...
    python -m jobs.twitter.co_occurrence \
        ...
        --output "topics.csv" \
        --lang "en" \
        --topics '[{"topic":"military", "words":["#army", "#navy", "#marines"]}]'

    # CNN Survey Example
    python -m jobs.twitter.co_occurrence \
        --input "data/surveys/*.json" \
        --output "topics" \
        --dataset "cnn_survey_2020" \
        --target_attr "awar1ot" \
        --topics_file "resources/20200813_Trump_Election2020_topics.json"
...
def udf_tokenize(text, lang=""):
    ...
    if text:
        if lang != "en":
            tokens = tokenize.multi_language_tokenizer(text, lang)
        else:
...
def flood_word_contenders(df):
    ...
    :returns: a list ['word']
    """
    max_value = df.select(F.max("count")).head()[0]
    ...
    flood_word_freq_cutoff = .02 * max_value
    int_freq_cutoff = int(flood_word_freq_cutoff)

    print("num rows before update:")
    print (df.count())

    df = df.filter(F.col("count") > int_freq_cutoff)

    print("num rows after update:")
    print (df.count())
    ...
    print("num rows before topic update:")
    print(df.count())

    df = df.filter( F.col("count") >= flood_word_topics_cutoff )

    print("num rows after topic update:")
    print(df.count())
